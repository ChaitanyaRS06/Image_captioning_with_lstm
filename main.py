...
def calculate_bleu(predicted_caption, real_caption):
    bleu_score=0
    r_caption=real_caption.split()  ## 1st is <start> , last is <end>
    r_caption=r_caption[1:-1]
    p_caption=predicted_caption.split()
    r_caption_set=[]
    p_caption_set=[]
    for i in range(len(r_caption)-1):
        r_caption_set.append((r_caption[i],r_caption[i+1]))

    for i in range(len(p_caption)-1):
        p_caption_set.append((p_caption[i],p_caption[i+1]))

    intersections= [ele1 for ele1 in p_caption_set for ele2 in r_caption_set if ele1 == ele2]
    unions=[]
    for i in range(len(p_caption)):
        x=tuple(set(p_caption[i] + r_caption[i]))
        unions.append(x)

    for i in range(len(p_caption_set)):
        intersection=intersections[i]
        union=unions[i]
        bleu_score +=bleu_score + (len(intersection) / len(union))
    
    return bleu_score

def calculate_bleu_library(predicted_caption, real_caption):
    r_caption=real_caption.split()
    r_caption=r_caption[1:-1]
    r_caption_new_list=[]
    r_caption_new_list.append(r_caption)
    p_caption=predicted_caption.split()
    print("r_caption_new_list:",r_caption_new_list)
    print("p_caption:",p_caption)
    bleu_score=nltk.translate.bleu_score.sentence_bleu(r_caption_new_list, p_caption,weights=(1, 0, 0, 0))
    return bleu_score
...
def main(COCO):
    ...
    train_seqs = tokenizer.texts_to_sequences(train_captions)  ## check this
    ...
    for i in range(3):
        ...
        #bleu_score=calculate_bleu(predicted_caption,real_caption)
        bleu_score=calculate_bleu_library(predicted_caption,real_caption)
